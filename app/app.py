...
def download_and_train():
    ...
    if not os.path.exists(data_path):
        ...
        if "label" not in df.columns:
            for col in ["class", "target", "fake"]:
                if col in df.columns:
                    df["label"] = df[col]; break
        label_map = {"fake":1,"FAKE":1,"Fake":1,"1":1,1:1,"real":0,"REAL":0,"Real":0,"0":0,0:0}
    ...
    st.info("🤖 Training model (~2–3 minutes on first launch)...", icon="⚙️")
    prog = st.progress(60, text="Loading data...")
    ...
    df = pd.read_csv(data_path)
    df["label"] = df["label"].astype(int)
    prog.progress(65, text="Preprocessing text...")
    sample = df.sample(min(len(df), 30_000), random_state=42).copy()
    sample["clean"] = sample["text"].apply(preprocess_text)
    prog.progress(75, text="Vectorising features...")

    X_train, X_test, y_train, y_test = train_test_split(
        sample["clean"], sample["label"], test_size=0.2,
        random_state=42, stratify=sample["label"]
    )
    vectoriser = TfidfVectorizer(max_features=100_000, ngram_range=(1,2),
                                  sublinear_tf=True, min_df=2, max_df=0.85)
    X_tr = vectoriser.fit_transform(X_train)
    X_te = vectoriser.transform(X_test)
    prog.progress(85, text="Fitting classifier...")

    model = LogisticRegression(max_iter=1000, C=5.0,
                                class_weight="balanced", random_state=42)
    model.fit(X_tr, y_train)
    prog.progress(95, text="Evaluating model...")
    ...
    for obj, fname in [(vectoriser,"tfidf_vectoriser.pkl"),
    ...
    prog.empty()
    st.success(f"✅ Model trained! Accuracy: {metrics['accuracy']:.1%} | F1: {metrics['f1']:.1%}", icon="🎉")
    st.rerun()
